(ns finishingops.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `finishingops.store` ns docstring for why a second (Datomic-
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:colorfastness-grade :grade-5}})
    (is (= :grade-5 (:colorfastness-grade (store/batch s "batch-001"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "equip-002" :concern-type :effluent-discharge
                                     :severity :high}})

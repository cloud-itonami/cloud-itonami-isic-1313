(ns finishingops.registry-test
  (:require [clojure.test :refer [deftest is]]
            [finishingops.registry :as r]))
...
(deftest known-grades-are-valid
  (doseq [g [:grade-5 :grade-4 :grade-3 :grade-2 :grade-1]]
    (is (r/grade-valid? g))))
...
;; ----------------------------- shrinkage-rate-valid? -----------------------------

(deftest typical-shrinkage-rate-is-valid
  (is (r/shrinkage-rate-valid? 2.0))
  (is (r/shrinkage-rate-valid? 0.0))
  (is (r/shrinkage-rate-valid? -3.0) "slight growth from heat-set/mercerizing finishing is plausible")
  (is (r/shrinkage-rate-valid? 20.0)))

(deftest excessive-growth-shrinkage-rate-is-invalid
  (is (not (r/shrinkage-rate-valid? -6.0))))

(deftest excessive-shrinkage-rate-is-invalid
  (is (not (r/shrinkage-rate-valid? 250.0)))
  (is (not (r/shrinkage-rate-valid? 20.01))))

(deftest non-numeric-or-missing-shrinkage-rate-is-invalid
  (is (not (r/shrinkage-rate-valid? nil)))
  (is (not (r/shrinkage-rate-valid? "2.0"))))

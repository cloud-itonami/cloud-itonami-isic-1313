(ns finishingops.operation-test
  "Smoke tests for the compiled FinishingOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `finishingops.governor-contract-test`; the Store contract in
  `finishingops.store-contract-test`."
...
(deftest test-actor-builds
  (testing "FinishingOperationActor can be built with a store"
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :concern-type :chemical-handling
                                   :severity :moderate :description "dye vat pH out of spec"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))

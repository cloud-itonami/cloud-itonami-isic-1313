(ns finishingops.operation
  "FinishingOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FinishingAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Textile
  Finishing Operations Governor (:govern) and the rollout phase gate
  ...
  each operation is auditable and checkpointed. A finishing plant's
  lifecycle is advanced by MANY ops (production-batch logging /
  maintenance scheduling / shipment coordination), each independent.
...
(defn build
  "Compiles a FinishingOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `finishingops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FinishingAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Textile Finishing Operations Governor -- independent censor.
      (g/add-node :govern

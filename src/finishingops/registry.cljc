(ns finishingops.registry
  "Pure-function domain logic for the textile-finishing plant-
  operations coordination actor -- equipment/batch verification,
  shipment-yardage recompute, colorfastness-grade validation,
  shrinkage-rate plausibility validation, and draft maintenance-
  schedule/shipment-coordination record construction.

  This vertical has NO pre-existing `kotoba-lang/textile-finishing`-
  ...
  functions, re-verified INDEPENDENTLY by `finishingops.governor` --
  the same 'ground truth, not self-report' discipline every sibling
  actor's own registry establishes (most directly
  `knittingops.registry`'s `shipment-volume-exceeded?`, this build's
  own closest domain analog): never trust a proposal's own self-
  reported yardage/status when the inputs needed to recompute it
  independently are already on record.

  This namespace is pure data + pure functions -- no I/O, no network
  call to any real plant-management system. It builds the DRAFT record
  a finishing-plant coordinator would keep (a scheduled maintenance
  window, a coordinated shipment back to the customer), not the act of
  actuating a dyeing/printing/bleaching/mercerizing line, or
...
(def valid-grades
  "The closed set of colorfastness-grade values a production-batch
  record may declare (post-finishing QC classification, mirroring the
  AATCC/ISO gray-scale colorfastness rating: 5 = no visible change,
  1 = severe change/fail). Anything else is a fabricated/unrecognized
  grade -- the governor HARD-holds rather than let an invented grade
  pass through."
  #{:grade-5 :grade-4 :grade-3 :grade-2 :grade-1})

(def shrinkage-rate-min-percent
  "Physical floor for a batch's own logged shrinkage-test reading.
  Finishing (especially heat-set/mercerizing) can slightly GROW a
  fabric rather than shrink it, so this floor is negative rather than
  zero -- a more negative reading than this is implausible test data,
  not a real batch."
  -5.0)

(def shrinkage-rate-max-percent
  "Physical ceiling for a batch's own logged shrinkage-test reading. A
  reading above this is implausible inspection/scale data, not a real
  finished-fabric batch."
  20.0)
...
(defn batch-verified?
  ...
  (i.e. its colorfastness-grade/finished-volume/shrinkage-rate claims
...
(defn batch-registered?
  ...
  production ledger? Coordinating a shipment back to the customer
  against a batch that is not on file and registered is the exact
  scope violation this actor's HARD invariant ('plant/batch record
  must be independently verified/registered before any action')
  exists to block."
  [batch]
...
(defn shipment-volume-exceeded?
  ...
  `shipped-to-date-yards` + `new-volume-yards` exceed `batch`'s own
  recorded `:volume-yards` (the batch's own logged finished-fabric
  yardage, post-shrinkage)? Needs no proposal inspection or
  ...
  cost/total-matching check uses (most directly
  `knittingops.registry/shipment-volume-exceeded?`, this build's own
  closest domain analog)."
...
(defn grade-valid?
  "Is `grade` one of the closed, known colorfastness-grade values?
...
(defn shrinkage-rate-valid?
  "Is `percent` a physically plausible batch-level shrinkage-test
  reading? Rejects nil, non-numbers, and values outside the plausible
  [-5.0, 20.0] range -- a fabricated or inspection-error reading,
  never let through as a real batch fact."
  [percent]
  (and (number? percent)
       (>= (double percent) shrinkage-rate-min-percent)
       (<= (double percent) shrinkage-rate-max-percent)))
...
(defn register-maintenance
  ...
  dyeing/printing/bleaching/mercerizing-line maintenance window against
  a verified, registered piece of equipment. Pure function -- does not
  actuate finishing-line equipment or execute any maintenance; it
  builds the RECORD a plant coordinator would keep.
  `finishingops.governor` independently re-verifies the equipment's own
...
(defn register-shipment
  ...
  finished-fabric shipment back to the customer against a verified,
  ...
  keep. `finishingops.governor` independently re-verifies the
  shipment's own claimed yardage against `shipment-volume-exceeded?`,

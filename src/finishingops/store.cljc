(ns finishingops.store
  "SSoT for the textile-finishing plant-operations coordination actor,
  ...
  external textile-finishing capability library, no jurisdiction-
  scoped Datomic parity requirement driving a second backend); a
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             dyeing/printing/bleaching/mercerizing
                             output on customer-supplied greige fabric:
                             colorfastness-grade/finished-volume/
                             shrinkage-rate record. `:verified?` marks
                             ...
                             `:shipped-volume-yards` tracks the batch's
                             own cumulative-shipped-back-to-customer
                             ground truth.
    - `equipment`         -- a dyeing-line/printing-line/bleaching-
                             line/mercerizing-line unit's own record.
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled dyeing/printing/bleaching/
                             mercerizing-line maintenance-window DRAFT
                             against a piece of equipment
                             (`finishingops.registry`'s
                             `register-maintenance`). Dedicated
                             `:scheduled?` double-schedule guard (never
                             a `:status` value).
    - `shipments`         -- a proposed finished-fabric shipment DRAFT
  ...
  unit, which shipment was coordinated back to the customer and at
  what independently-recomputed yardage, approved by whom, which
  safety concern was flagged' is always a query over an immutable log
  -- the audit trail a plant owner or fabric customer trusting this
  coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (finishingops.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (finishingops.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :process-type :dyeing :customer-id "customer-acme"
                :fabric "cotton-poplin" :colorfastness-grade :grade-5
                :volume-yards 20000.0 :shrinkage-percent 1.5
                :verified? true :registered? true
                :shipped-volume-yards 5000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :process-type :printing :customer-id "customer-nishijin"
                :fabric "polyester-satin" :colorfastness-grade :grade-4
                :volume-yards 6000.0 :shrinkage-percent 2.0
                :verified? true :registered? true
                :shipped-volume-yards 5700.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :process-type :bleaching :customer-id "customer-hakusen"
                :fabric "cotton-canvas" :colorfastness-grade :grade-2
                :volume-yards 15000.0 :shrinkage-percent 8.0
                :verified? false :registered? false
                :shipped-volume-yards 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :dyeing-line
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :mercerizing-line
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `finishingops.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `finishingops.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defrecord MemStore [a]
  ...
  (commit-record! [s {:keys [effect path value] :as record}]
    (cond
      ...
      ;; above (also what `knittingops`-style siblings expose as their
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered dyeing-line unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  mercerizing-line unit (blocks any maintenance scheduling against
